...
import com.company.automatfinit.AF_CONST;
import com.company.automatfinit.AF_ID;
...
public class Main {
    ...
    private static final AF_ID afID = new AF_ID();
    private static final AF_CONST afCONST = new AF_CONST();
    ...
    private static boolean isID(String atom) {
        return afID.isID(atom);
    }

    private static boolean isCONST(String atom) { return afCONST.isCONST(atom); }
    ...
    private static int addToTS(String element) {
        if (ts.find(element) != null){
            return Pair.codAtomTS;
        }
        if (ts.size() == 0) {
            ts.add(new Pair(element, Pair.codAtomTS), 0);
            Pair.codAtomTS++;
        }
            ...
            ts.add(new Pair(element, Pair.codAtomTS), ts.size());
            Pair.codAtomTS++;
        }
        else {
            int i = 0;
            while (element.compareTo(ts.get(i).getSimbol()) > 0) i++;
            ts.add(new Pair(element, Pair.codAtomTS), i);
            Pair.codAtomTS++;
        }
        return ts.size();
    }
