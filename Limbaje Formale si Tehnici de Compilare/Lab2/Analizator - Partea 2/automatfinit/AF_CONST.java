...
public class AF_CONST {
    ...
    public AF_CONST() {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader("afCONST.txt"))) {
            ...
            System.out.println("Error from AF_CONST():");
            e.printStackTrace();
        }
    }
    ...
    public boolean isCONST(String sequence) {
        ...
        if(sequence.length() > 1 && sequence.charAt(0) == '0') {
            return false;
        }
        ...
        for (int i = 0; i < sequence.length() && ok; i++) {
            ...
            for (Tranzitie tranzitie : tranzitii) {
                if (tranzitie.getStareInitiala().equals(stareInitiala)) {
                    ...
                    if (!tranzitie.getStareInitiala().equals(this.stareInitiala) && tranzitie.getValoare().equals(".")) {
                        if (character.equals(".")) {
                            if (dotFound)
                                return false;
                            dotFound = true;
